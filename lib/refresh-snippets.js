    ...
    for (var i = 0; i < this.projectsInWorkspace.length; i++) {
      ...
      for (var j = 0; j < node_modules.length; j++) {
        if(!(node_modules[j].startsWith("ember-")))
          continue
        try {
          var module_snippets_json = JSON.parse(fs.readFileSync(this.projectsInWorkspace[i] +
                                                                '/node_modules/' +
                                                                node_modules[j] +
                                                                "/snippets/" +
                                                                "snippets.json"))
          ...
          console.log(this.projectsInWorkspace[i] +
                                                                ...
                                                                "/snippets/" +
                                                                "snippets.json")
          console.log("Something went wrong when opening " + node_modules[j] + ". It may not have any snippets.")
          continue;
        }
